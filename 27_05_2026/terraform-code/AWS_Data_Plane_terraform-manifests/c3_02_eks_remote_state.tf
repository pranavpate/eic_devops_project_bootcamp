...
data "terraform_remote_state" "eks" {
  ...
  config = {
    bucket  = "tfstate-dev-ap-south-1-5n72px"     # Name of the remote S3 bucket where the EKS state is stored
    key     = "deveks/terraform.tfstate"        # Path to the EKS tfstate file within the bucket
    region  = "ap-south-1"                    # Region where the S3 bucket exist
