...
data "terraform_remote_state" "vpc" {
  ...
  config = {
    bucket = "tfstate-dev-ap-south-1-5n72px"     # Name of the remote S3 bucket where the VPC state is stored
    key    = "dev/terraform.tfstate"        # Path to the VPC tfstate file within the bucket
    region = "ap-south-1"                    # Region where the S3 bucket and DynamoDB table exist
